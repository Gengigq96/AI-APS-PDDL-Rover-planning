(define (domain Rover2)
(:requirements :typing :strips)
(:types rover waypoint store camera mode lander objective battery-number)
...
(:predicates (at ?x - rover ?y - waypoint) 
	     ...
	     (channel_free ?l - lander)
            (battery ?r - rover ?b1 - battery-number)
            (battery-predecessor ?b1 ?b2 - battery-number)

)

	
(:action navigate
:parameters (?x - rover ?y - waypoint ?z - waypoint ?b1 - battery-number ?b2 - battery-number) 
:precondition (and 
                     (can_traverse ?x ?y ?z) 
                     (available ?x) 
                     (at ?x ?y) 
                     (visible ?y ?z)
                     (battery-predecessor ?b1 ?b2)
                     (battery ?x ?b2)
	    )
:effect (and (not 
              (at ?x ?y)) 
...
(:action recharge
:parameters (?x - rover ?l - lander ?y - waypoint ?b1 - battery-number ?b2 - battery-number)
:precondition (and 
              (at ?x ?y)
              (at_lander ?l ?y)
              (battery-predecessor ?b1 ?b2)
